...
(function($) {
    $.imageArea = function(parent, id) {
        var options = parent.options,
            ...
            updateLblInfo = function (visible) {
              if ($lblinfo) {
                ...
                $lblinfo.css({
                  ...
                  var node = $("<select>").appendTo($lblinfo);
                  $(Object.keys(options.labelsDict)).each(function(){

                    //console.log(area.lblid + " " + options.defaultLabelId + " " + this);
                    ...
                    if(area.lblid === null &&  this == options.defaultLabelId){
                      ...
                      node.append($("<option>").attr('value', this).text(options.labelsDict[this]));
                    }
                  });
                  $(node).on('change', function(){
                    area.lblid = this.value;
                    area.lbltxt = options.labelsDict[this.value];

                    //console.log("area [" + area.id + "] changed to: " + area.lblid + " " + area.lbltxt);
                    fireEvent("changed");
        ...
        //init info label
        $lblinfo = $("<div class=\"info-area\"></div>").insertAfter($selection);
        ...
        return {
            ...
            set: function (dimensions, silent) {
                ...
                // else{
                //   console.log("not raising event on changed " + JSON.stringify(dimensions));
                // }
            },
    ...
    $.imageSelectAreas.prototype.setDefaultLabel = function (newDefaultLabelId) {
      this.options.defaultLabelId = newDefaultLabelId;
    };
